...
function readBlockValue(body, blockName, key) {
  ...
  const doubleQuotedRegex = new RegExp(`^  ${key}:[ \\t]*"((?:[^"\\\\]|\\\\.)*)"`);
  const singleQuotedRegex = new RegExp(`^  ${key}:[ \\t]*'((?:[^'\\\\]|\\\\.)*)'`);
  const bareRegex = new RegExp(`^  ${key}:[ \\t]*([^\\n#]*)`);

  for (let i = headerIndex + 1; i < nextBlockIndex; i++) {
    const dQuoted = lines[i].match(doubleQuotedRegex);
    if (dQuoted) {
      return dQuoted[1].replaceAll('\\"', '"').replaceAll('\\\\', '\\').trim();
    }
    const sQuoted = lines[i].match(singleQuotedRegex);
    if (sQuoted) {
      return sQuoted[1].replaceAll("''", "'").trim();
    }
