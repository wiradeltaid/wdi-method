...
[BMad](https://github.com/bmad-code-org/BMAD-METHOD) пишет документы для ИИ-агентов. WDI Method добавляет документы, которые многие роли уже читают: сценарии использования, диаграммы C4, перечни API и баз данных, проектные документы. Он оборачивает BMad, не заменяя его: каждый навык WDI передаёт написание навыку BMad, а затем проверяет результат по руководствам метода.
...
- **Вы набираете его**: четыре навыка daily tier, `wdi-build` и `wdi-explain-to-me` (у них есть `disable-model-invocation: true`).
...
| `/wdi-build` | G5. Одна спецификация от открытия до закрытия: вы запускаете `to-spec` и `to-tickets`, каждый тикет доходит до зелёного PR, затем спецификация закрывается. Никогда не сливает. | Вы набираете его |
| **Daily tier** | | |
